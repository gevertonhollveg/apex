    ...
    function update() {
        ...
        if (openTime > 0) {
            if (offset - timeLeft < openTime) {
                document.getElementById(eventId).innerHTML = '<span style="color:var(--accent-green);font-weight:600">Open</span>';
                timeLeft--;
                return;
            }
        } else {
            if (duration > 0) {
                if (offset - timeLeft < duration) {
                    document.getElementById(eventId).innerHTML = '<span style="color:var(--accent-blue);font-weight:600">In Progress</span>';
                    timeLeft--;
                    return;
                }
            }
        ...
        if (eHours == '00' && eMinutes == '00') {
            ...
            if (eDays > 0) {
                document.getElementById(eventId).innerHTML = eDays + "d " + eHours + "h " + eMinutes + "m " + eSeconds + "s";
            } else {
                document.getElementById(eventId).innerHTML = eHours + "h " + eMinutes + "m " + eSeconds + "s";
            }
        }
        timeLeft--;
    }
